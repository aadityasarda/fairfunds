...
const getAllExpensesForUser = async (req, res) => {
  ...
  try {
    ...
    // ✅ STEP 1: Process active expenses
    ...
    // ✅ STEP 2: Apply only settlements that are NOT orphaned
    ...
    // ✅ STEP 4: Filter only balances related to current user
    const result = Object.values(netBalances).filter(
      b => b.from === userId || b.to === userId
    );

    // ✅ STEP 5: Return member info for frontend
